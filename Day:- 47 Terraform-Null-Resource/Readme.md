...
1. Providers:
   ...
   - Null Provider: It provides resources like null_resource, mostly used for provisioners or forcing changes without affecting infrastructure.
     
   - Time Provider (time_sleep): Useful when the target resource (like a VM) needs some time to finish initialization.
...
Terraform detects change because the timestamp forces replacement:
