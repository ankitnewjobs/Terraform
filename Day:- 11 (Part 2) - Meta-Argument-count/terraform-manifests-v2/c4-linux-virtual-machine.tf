...
- **Value**: The username is set as `azureuser`.
...
- **Key Attributes**:
  ...
  - **`caching`**: Set to `ReadWrite` for improved performance in read and write operations.
  - **`storage_account_type`**: Disk type is `Standard_LRS` (Locally Redundant Storage), cost-effective and reliable.
