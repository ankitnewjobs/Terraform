...
- **`address_space`:** Defines the IP address range in CIDR format for the virtual network. Here, `10.0.0.0/16` allows for 65,536 IP addresses within the VNet.
...
This resource sets up the primary private network for Azure workloads.
...
This isolates a segment of the VNet for resource grouping and network management.
...
Provides a public-facing IP for resources (like VMs) to be accessible from the Internet.
...
- **`tags`:** Common tags for resource management.
